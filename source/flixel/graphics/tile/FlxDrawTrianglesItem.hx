...
class FlxDrawTrianglesItem extends FlxDrawBaseItem<FlxDrawTrianglesItem>
{
  ...
  override public function reset():Void
  {
    ...
    if (colorMultipliers != null) colorMultipliers.splice(0, colorMultipliers.length);
    if (colorOffsets != null) colorOffsets.splice(0, colorOffsets.length);
    #end
  }
      ...
      ?cameraBounds:FlxRect #if !flash, ?transform:ColorTransform #end):Void
  {
    ...
    if (cameraBounds == null) cameraBounds = rect.set(0, 0, FlxG.width, FlxG.height);
  ...
  public function addTriangles2(vertices:DrawData<Float>, indices:DrawData<Int>, uvtData:DrawData<Float>, ?colors:DrawData<Int>, ?position:FlxPoint,
      ?cameraBounds:FlxRect #if !flash, ?transform2:Array<ColorTransform> #end):Void
  {
    if (position == null) position = point.set();
    ...
    if (colored || hasColorOffsets)
    {
      ...
      for (i in 0...Std.int(indicesLength / 3))
      {
        var transform = transform2[i];
        for (_ in 0...3)
        {
          if (transform != null)
          {
