...
class CosineSpline
{
  ...
  public function evaluate(t:Float, loop:Bool):Float
  {
    ...
    var next:Float = points[p][1];
    var cosFactor:Float = (1.0 - Math.cos(tfrac * Math.PI)) / 2.0;
    return current + (next - current) * cosFactor;
  }
  ...
  {
    if (i >= points.length) throw "CosineSpline::set_point requires the index to be less than the number of points.";
    points[i][0] = v;
  }

  public function set_coefficients(i:Int, b:Float):Void
  {
    if (i >= points.length) throw "CosineSpline: point index must be less than the number of points.";
    points[i][1] = b;
  }
...
class CosineSplineN
{
  ...
  public var splines:Array<CosineSpline> = [];
  ...
  public function weighted_average(out:CosineSplineN, from:CosineSplineN, to:CosineSplineN, between:Float):Void
  {
    ...
    for (spli in 0...out.splines.length)
    {
      for (p in 0...out_size)
      {
        ...
        if (p < from_size) fc = from.splines[spli].get_point_and_coefficients(p);
        if (p < to_size) tc = to.splines[spli].get_point_and_coefficients(p);
        else
          for (i in 0...2)
            tc[i] = fc[i];
        if (p >= from_size) for (i in 0...2)
          fc[i] = tc[i];

        var oc:Array<Float> = [0.0, 0.0];
        oc[0] = ModchartMath.lerp(between, fc[0], tc[0]);
        oc[1] = ModchartMath.lerp(between, fc[1], tc[1]);
  ...
  {
    if (v.length != splines.length) throw "CosineSplineN::set_point requires the passed point to be the same dimension as the spline.";
  ...
  {
    if (b.length != splines.length) throw "CosineSplineN: coefficient vector must be the same dimension as the spline.";
    for (n in 0...splines.length)
  ...
  {
    if (i >= splines.length) throw "CosineSplineN: index of spline to set extent of is out of range.";
  ...
  public function size():Int
  {
    return splines.length > 0 ? splines[0].size() : 0;
  }
  ...
  public function redimension(d:Int):Void
  {
    ...
    {
      if (splines[i] == null) splines[i] = new CosineSpline();
    }
    dirty = true;
  }
...
class CosineSplineHandler
{
  public var spline:CosineSplineN;
  public var subtract_song_beat_from_curr:Bool = true;

  public function new()
  {
    spline = new CosineSplineN();
  }
  ...
  public function MakeWeightedAverage(out:CosineSplineHandler, from:CosineSplineHandler, to:CosineSplineHandler, between:Float)
  {
